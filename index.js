...
async function loadContact(contact) {
  ...
  const appData = await ManageAppQuery(contact.handle);
  displayAppInfo(appData.info.id, appData.info.location, appData.info.status);

  //  Load the notes from our CRM data.
  displayNotes(appData.page);
}
...
function displayAppInfo (id = "-", location = "-", status = "-") {
